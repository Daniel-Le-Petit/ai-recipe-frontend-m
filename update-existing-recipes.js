...
async function updateExistingRecipes() {
  ...
  try {
    ...
    if (existingRecipesData.data && existingRecipesData.data.length > 0) {
      console.log('\n3. Mise à jour des recettes incomplètes...');
      let updatedCount = 0;
      
      // Mettre à jour les premières recettes avec des données complètes
      const recipesToUpdate = Math.min(existingRecipesData.data.length, recipeUpdates.length);
      
      for (let i = 0; i < recipesToUpdate; i++) {
        const recipe = existingRecipesData.data[i];
        const updateData = recipeUpdates[i];
        
        console.log(`   Mise à jour de la recette ${recipe.id}: "${updateData.title}"`);
        
        const updateResponse = await fetch(`${API_URL}/api/recipies/${recipe.id}`, {
          method: 'PUT',
          headers: {
        ...
        if (updateResponse.ok) {
          updatedCount++;
          console.log(`   ✅ Recette ${recipe.id} mise à jour`);
        } else {
          console.log(`   ❌ Erreur lors de la mise à jour de ${recipe.id}: ${updateResponse.status}`);
        }
      }
    ...
    if (finalRecipesData.data && finalRecipesData.data.length > 0) {
      ...
      finalRecipesData.data.forEach((recipe, index) => {
        ...
        console.log(`   Instructions: ${recipe.attributes?.instructions ? '✅ Présentes' : '❌ Manquantes'}`);
      });
    }
