...
import { StrapiRecipe, StrapiWeeklyPlan, WeeklyPlanData } from '../../types/api'
import { useWeeklyPlans } from '../../hooks/useRecipes'

export default function MesPlansSemainesPage() {
  const [currentWeekIndex, setCurrentWeekIndex] = useState(0)
  const [recipes, setRecipes] = useState<StrapiRecipe[]>([])
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)

  // Utiliser le hook pour les plans de semaine
  const { plans: strapiWeeklyPlans, loading: plansLoading, error: plansError } = useWeeklyPlans()

  // Fonction pour récupérer les recettes depuis Strapi
  const fetchRecipes = async () => {
    try {
      ...
      const response = await fetch(`${API_URL}/api/recipies?populate=*&filters[recipeState][$eq]=approved`)
  ...
  // Fonction pour transformer une recette Strapi en format de la page
  const transformRecipe = (recipe: StrapiRecipe) => {
    return {
      name: recipe.attributes.title,
      time: `${recipe.attributes.duration || 15} min`,
      portions: `${recipe.attributes.servings || 2} pers.`,
      rating: recipe.attributes.rating?.toString() || '4.2',
      tags: recipe.attributes.tags || ['Vegetarien', 'Sain'],
      ingredients: recipe.attributes.ingredients?.map((ing: any) => ing.name || ing) || ['Quinoa', 'Légumes', 'Huile'],
      status: 'accepted' as const
    }
  }

  // Données de base pour les plans hebdomadaires (fallback)
  const fallbackWeeklyPlans = [
    {
  ...
  // Fonction pour transformer les plans Strapi en format d'affichage
  const transformWeeklyPlans = () => {
    if (strapiWeeklyPlans.length === 0) return fallbackWeeklyPlans

    return strapiWeeklyPlans.map(plan => ({
      weekStart: plan.attributes.weekStart,
      selections: plan.attributes.selections,
      days: [
        { date: '21-Jul-2025', day: 'Lundi 21 Juillet' },
        { date: '22-Jul-2025', day: 'Mardi 22 Juillet' },
        { date: '23-Jul-2025', day: 'Mercredi 23 Juillet' },
      ...
      meals: [
           ...
           name: 'Petit-Déjeuner',
           icon: '🌿',
           recipes: [{ ...transformRecipe(recipes[0] || recipes[0]), status: 'accepted' as const }]
         },
         {
           name: 'Déjeuner',
           icon: '🥗',
           recipes: [{ ...transformRecipe(recipes[1] || recipes[0]), status: 'accepted' as const }]
         },
         {
           name: 'Collation',
           icon: '🥜',
           recipes: [{ ...transformRecipe(recipes[2] || recipes[0]), status: 'declined' as const }]
         },
         {
           name: 'Dîner',
           icon: '🍽️',
           recipes: [{ ...transformRecipe(recipes[3] || recipes[0]), status: 'accepted' as const }]
         }
      ]
    }))
  }

  const weeklyPlans = transformWeeklyPlans()
              ...
              {loading && (
                <div style={{
                  ...
                  <p style={{
                    ...
                    🔄 Chargement des recettes depuis Strapi...
                  </p>
                </div>
              )}
